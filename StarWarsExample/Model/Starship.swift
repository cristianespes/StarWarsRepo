...
func returnArrayOfAllStarshipsFromData(result: [AnyObject]) -> [Starship] {
    return result.map { generateStarshipFromJsonData(dataStarship: $0) }.sorted { $0.name < $1.name }
}
...
func getArrayOfPlanetsFromID(id: Int, result: [AnyObject]) -> Starship? {
    ...
    return result.filter( { ($0["id"] as! Int) == id } ).map { generateStarshipFromJsonData(dataStarship: $0) }.first ?? nil
