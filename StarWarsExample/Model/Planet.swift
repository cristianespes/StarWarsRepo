...
func returnArrayOfAllPlanetsFromData(result: [AnyObject]) -> [Planet] {
    return result.map { generatePlanetFromJsonData(dataPlanet: $0) }.sorted { $0.name < $1.name }
}
...
func getPlanetByID(id: Int, result: [AnyObject]) -> Planet? {
    
    return result.filter( { ($0["id"] as! Int) == id } ).map { generatePlanetFromJsonData(dataPlanet: $0) }.first ?? nil
